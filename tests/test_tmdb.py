...
from tests.samples.tmdb_mock_sample import (
    AOT_FIND_MOCK,
    AOT_TRANSLATIONS_MOCK,
    TSOTG_FIND_MOCK,
    TSOTG_TRANSLATIONS_MOCK,
)
...
async def test_tmdb_not_found(provider: TMDBMetadataProvider):
    ...
    respx.get(f"{provider.base_url}/find/{imdb_id}").respond(
        status_code=200,
        text=json.dumps({"movie_results": [], "tv_results": []}),
    )
    query = MetadataQuery(type=MediaType.MOVIE, id=imdb_id)
...
async def test_tmdb_timeout_on_details(provider: TMDBMetadataProvider):
    ...
    respx.get(f"{provider.base_url}/find/{imdb_id}").respond(status_code=200, text=TSOTG_FIND_MOCK)
    respx.get(f"{provider.base_url}/movie/{tsotg_find_json['movie_results'][0]['id']}").side_effect = (
        httpx.ReadTimeout("TMDB is taking too long to respond")
    )
